...
class KalmanFilter(Q_metres_per_second: Float) {

    private val MinAccuracy = 1f
    private var TimeStamp_milliseconds: Long = 0
    private var lat = 0.0
    private var lng = 0.0
    private var variance = -1f  // P matrix. Negative means object uninitialised.

    // NB: units irrelevant, as long as same units used
    // throughout
    var consecutiveRejectCount = 0
    private var qMeterPerSecond: Float = Q_metres_per_second
    ...
    fun Process(
        ...
        qMeterPerSecond = Q_metres_per_second
        ...
        if (variance < 0) {
            ...
            if (TimeInc_milliseconds > 0) {
                ...
                // increases
                variance += (TimeInc_milliseconds * qMeterPerSecond
                        * qMeterPerSecond) / 1000
                this.TimeStamp_milliseconds = TimeStamp_milliseconds
