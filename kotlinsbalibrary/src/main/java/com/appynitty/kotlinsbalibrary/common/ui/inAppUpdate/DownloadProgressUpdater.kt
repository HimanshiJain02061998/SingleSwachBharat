...
internal class DownloadProgressUpdater(
    ...
    interface DownloadProgressListener {
        fun updateProgress(progress: Long , downloadId: Long)
    }
                ...
                if (it.moveToFirst()) {
                    ...
                    when (downloadStatus) {
                        DownloadManager.STATUS_SUCCESSFUL -> {
                            downloadProgressListener?.updateProgress(DOWNLOAD_COMPLETE,downloadId)

                            return

                        }
                        DownloadManager.STATUS_FAILED -> {
                            downloadProgressListener?.updateProgress(DOWNLOAD_FAILED,downloadId)
                            return
                        }
                        else -> {
                            //update progress
                            val percentProgress = ((bytesDownloadedSoFar * 100L) / totalBytes)
                            downloadProgressListener?.updateProgress(percentProgress , downloadId)
                        }
                    }
